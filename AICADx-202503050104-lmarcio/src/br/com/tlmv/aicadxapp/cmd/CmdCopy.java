...
public class CmdCopy implements CmdBase, Runnable
{
//Private
	...
	public void run() {
		...
		if(oEnt != null) {
			if(oEnt.getObjType() == AppDefs.OBJTYPE_POINT) {
				...
				CadPoint otherPoint = (CadPoint)o.copyTo(ptI, ptF);
				currBlockDef.addEntity(otherPoint);
			}
			else if(oEnt.getObjType() == AppDefs.OBJTYPE_LINE) {
				CadLine o = (CadLine)oEnt;

				CadLine otherLine = o.copyTo(ptI, ptF);
				currBlockDef.addEntity(otherLine);
			}
			else if(oEnt.getObjType() == AppDefs.OBJTYPE_RECTANGLE) {
				CadRectangle o = (CadRectangle)oEnt;

				CadRectangle otherRectangle = o.copyTo(ptI, ptF);
				currBlockDef.addEntity(otherRectangle);
			}
			else if(oEnt.getObjType() == AppDefs.OBJTYPE_CIRCLE) {
				CadCircle o = (CadCircle)oEnt;

				CadCircle otherCircle = o.copyTo(ptI, ptF);
				currBlockDef.addEntity(otherCircle);
			}
			else if(oEnt.getObjType() == AppDefs.OBJTYPE_ARC) {
				CadArc o = (CadArc)oEnt;

				CadArc otherArc = o.copyTo(ptI, ptF);
				currBlockDef.addEntity(otherArc);
			}
			else if(oEnt.getObjType() == AppDefs.OBJTYPE_TEXT) {
				CadText o = (CadText)oEnt;

				CadText otherText = o.copyTo(ptI, ptF);
				currBlockDef.addEntity(otherText);
			}
		}
