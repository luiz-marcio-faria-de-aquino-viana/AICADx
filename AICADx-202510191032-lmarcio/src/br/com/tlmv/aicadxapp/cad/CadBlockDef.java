...
//Private
	...
	private ArrayList<CadEntity> findAllEntityByObjTypeAndSearchString(ArrayList<Integer> ls, int objType, String searchString) {
		ArrayList<CadEntity> lsResult = new ArrayList<CadEntity>();		
		for(Integer objectId : ls) {
			CadEntity oEnt = this.entityTable.get(objectId);
			if(oEnt.isDeleted()) continue;
...
//Public
	...
	public CadBlockDef() {
		super(AppDefs.OBJTYPE_BLOCK_DEF, null);
	}
	...
	public synchronized ArrayList<CadEntity> findAllEntityByObjType(int objType, String searchString) {
		ArrayList<CadEntity> lsResult = new ArrayList<CadEntity>();		
		for(CadEntity oEnt : this.lsEntity) {
			if( oEnt.isDeleted() ) continue;
			if( oEnt.search(objType, searchString) ) {
	...
	public synchronized ArrayList<CadEntity> findAllVisibleEntityByObjType(int objType, String searchString) {
		return findAllEntityByObjTypeAndSearchString(this.lsVisibleEntity, objType, searchString);
	}	
	...
	public synchronized ArrayList<CadEntity> findAllSelectedEntityByObjType(int objType, String searchString) {
		return findAllEntityByObjTypeAndSearchString(this.lsSelectedEntity, objType, searchString);
	}	
	...
	public GeomDimension3d getEnvelop3d(int objType) {
		GeomDimension3d oGeomDim = GeomUtil.getEnvelop3d(this.lsEntity, objType);
		return oGeomDim;
	}
	
	public GeomDimension2d getEnvelop2d(int objType) {
		GeomDimension2d oGeomDim = GeomUtil.getEnvelop2d(this.lsEntity, objType);
		return oGeomDim;
	}
