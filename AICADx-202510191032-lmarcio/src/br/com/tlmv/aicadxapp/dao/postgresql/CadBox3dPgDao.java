 ...
 * CadBox3dPgDao.java
 * Autor: 
 *   Luiz Marcio Faria de Aquino Viana, Pos-D.Sc. - Engenheiro, 22/06/2025
...
public class CadBox3dPgDao extends CadEntityBaseDao
{
//Public
	
	/* Constructors */
	
	public CadBox3dPgDao(BaseDao dao)
	{
		super(dao);
	}
	...
	public BaseObjectRecord selectByPk(String schemaName, Integer objectId)
	{
		...
		try {
			String sql = StringUtil.replace(CadBox3dRecord.sqlSelectByPk, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public ArrayList<BaseObjectRecord> selectAll(String schemaName)
	{
		...
		try {
			String sql = StringUtil.replace(CadBox3dRecord.sqlSelectAll, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			ResultSet rs = stmt.executeQuery();
			while( rs.next() ) {
				CadBox3dRecord o = new CadBox3dRecord(rs);
				lsResult.add(o);
			}
		}
		catch(Exception e) {
	...
	public ArrayList<BaseObjectRecord> selectByRefEntityId(String schemaName, String refEntity) 
	{
		...
		try {
			String sql = StringUtil.replace(CadBox3dRecord.sqlSelectByReference, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer insert(String schemaName, BaseObjectRecord o) {
		CadBox3dRecord oEnt = (CadBox3dRecord)o;		
		
		BaseDao dao = this.getDao();
		Integer result = -1;
		
		try {
			String sql = StringUtil.replace(CadBox3dRecord.sqlInsert, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			stmt.setDouble(n++, oEnt.getAltura());
	...
	public Integer update(String schemaName, BaseObjectRecord o) {
		...
		try {
			String sql = StringUtil.replace(CadBox3dRecord.sqlUpdate, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
