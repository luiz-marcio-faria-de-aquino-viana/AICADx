...
public class CadPipePgDao extends CadEntityBaseDao
{
//Public
	
	/* Constructors */
	
	public CadPipePgDao(BaseDao dao)
	{
		super(dao);
	}
	...
	public BaseObjectRecord selectByPk(String schemaName, Integer objectId)
	{
		...
		try {
			String sql = StringUtil.replace(CadPipeRecord.sqlSelectByPk, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public ArrayList<BaseObjectRecord> selectAll(String schemaName)
	{
		...
		try {
			String sql = StringUtil.replace(CadPipeRecord.sqlSelectAll, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			ResultSet rs = stmt.executeQuery();
			while( rs.next() ) {
				CadPipeRecord o = new CadPipeRecord(rs);
				lsResult.add(o);
			}
		}
		catch(Exception e) {
	...
	public ArrayList<BaseObjectRecord> selectByRefEntityId(String schemaName, String refEntity) 
	{
		...
		try {
			String sql = StringUtil.replace(CadPipeRecord.sqlSelectByReference, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer insert(String schemaName, BaseObjectRecord o) {
		CadPipeRecord oEnt = (CadPipeRecord)o;		
		
		BaseDao dao = this.getDao();
		Integer result = -1;
		
		try {
			String sql = StringUtil.replace(CadPipeRecord.sqlInsert, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	    	...
	    	stmt.setInt(n++, oEnt.getNumeroPipe());
	    	stmt.setInt(n++, oEnt.getFromObjectId());
	    	stmt.setInt(n++, oEnt.getToObjectId());
	    	stmt.setString(n++, oEnt.getSectionType());
	    	stmt.setString(n++, oEnt.getPipeCategory());
	    	stmt.setDouble(n++, oEnt.getDiameter());
	    	stmt.setDouble(n++, oEnt.getWidth());
	    	stmt.setDouble(n++, oEnt.getHeight());
	    	stmt.setDouble(n++, oEnt.getThickness());
	    	stmt.setDouble(n++, oEnt.getMaxPipeSegmentLength());
	    	stmt.setDouble(n++, oEnt.getExternalDiameter());
	    	stmt.setDouble(n++, oEnt.getExternalWidth());
	    	stmt.setDouble(n++, oEnt.getExternalHeight());
	...
	public Integer update(String schemaName, BaseObjectRecord o) {
		...
		try {
			String sql = StringUtil.replace(CadPipeRecord.sqlUpdate, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
