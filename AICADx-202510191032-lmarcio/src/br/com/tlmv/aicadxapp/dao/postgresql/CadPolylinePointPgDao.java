...
public class CadPolylinePointPgDao extends BasePointDao 
{
//Public
	
	/* Constructors */
	
	public CadPolylinePointPgDao(BaseDao dao)
	{
		super(dao);
	}
	...
	{
		CadPolylinePointRecord result = null;
		
		BaseDao dao = this.getDao();
		
		try {
			String sql = StringUtil.replace(CadPolylinePointRecord.sqlSelectByPk, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			if( rs.next() )
				result = new CadPolylinePointRecord(rs);
		}
		catch(Exception e) {
			e.printStackTrace();
		}		
	...
	public ArrayList<BasePointRecord> selectAll(String schemaName)
	{
		...
		try {
			String sql = StringUtil.replace(CadPolylinePointRecord.sqlSelectAll, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			ResultSet rs = stmt.executeQuery();
			while( rs.next() ) {
				CadPolylinePointRecord o = new CadPolylinePointRecord(rs);
				lsResult.add(o);
			}
		}
		catch(Exception e) {
	...
	public ArrayList<BasePointRecord> selectByRefEntityId(String schemaName, int refEntityId) 
	{
		...
		try {
			String sql = StringUtil.replace(CadPolylinePointRecord.sqlSelectByCadPolylineId, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer insert(String schemaName, BasePointRecord o) {
		CadPolylinePointRecord oEnt = (CadPolylinePointRecord)o;		
		
		BaseDao dao = this.getDao();
		Integer result = -1;
		
		try {
			String sql = StringUtil.replace(CadPolylinePointRecord.sqlInsert, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer update(String schemaName, BasePointRecord o) {
		...
		try {
			String sql = StringUtil.replace(CadPolylinePointRecord.sqlUpdate, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
