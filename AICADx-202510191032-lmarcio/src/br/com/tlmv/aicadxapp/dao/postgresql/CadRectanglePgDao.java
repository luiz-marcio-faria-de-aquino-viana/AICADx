 ...
 *
 * CadRectanglePgDao.java
 * Autor: 
 *   Luiz Marcio Faria de Aquino Viana, Pos-D.Sc. - Engenheiro, 23/06/2025
...
public class CadRectanglePgDao extends CadEntityBaseDao
{
//Public
	
	/* Constructors */
	
	public CadRectanglePgDao(BaseDao dao)
	{
		super(dao);
	}
	...
	public BaseObjectRecord selectByPk(String schemaName, Integer objectId)
	{
		...
		try {
			String sql = StringUtil.replace(CadRectangleRecord.sqlSelectByPk, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			if( rs.next() )
				result = new CadRectangleRecord(rs);
		}
		catch(Exception e) {
			e.printStackTrace();
		}		
	...
	public ArrayList<BaseObjectRecord> selectAll(String schemaName)
	{
		...
		try {
			String sql = StringUtil.replace(CadRectangleRecord.sqlSelectAll, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			ResultSet rs = stmt.executeQuery();
			while( rs.next() ) {
				CadRectangleRecord o = new CadRectangleRecord(rs);
				lsResult.add(o);
			}
		}
		catch(Exception e) {
	...
	public ArrayList<BaseObjectRecord> selectByRefEntityId(String schemaName, String refEntity) 
	{
		...
		try {
			String sql = StringUtil.replace(CadRectangleRecord.sqlSelectByReference, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer insert(String schemaName, BaseObjectRecord o) {
		CadRectangleRecord oEnt = (CadRectangleRecord)o;		
		
		BaseDao dao = this.getDao();
		Integer result = -1;
		
		try {
			String sql = StringUtil.replace(CadRectangleRecord.sqlInsert, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			stmt.setDouble(n++, oEnt.getPtMinX());
			stmt.setDouble(n++, oEnt.getPtMinY());
			stmt.setDouble(n++, oEnt.getPtMinZ());
			stmt.setDouble(n++, oEnt.getPtMaxX());
			stmt.setDouble(n++, oEnt.getPtMaxY());
			stmt.setDouble(n++, oEnt.getPtMaxZ());
	...
	public Integer update(String schemaName, BaseObjectRecord o) {
		...
		try {
			String sql = StringUtil.replace(CadRectangleRecord.sqlUpdate, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
