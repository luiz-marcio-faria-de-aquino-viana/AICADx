...
public class CadCilinder3dPgDao extends CadEntityBaseDao
{
//Public
	
	/* Constructors */
	
	public CadCilinder3dPgDao(BaseDao dao)
	{
		super(dao);
	}
	...
	public BaseObjectRecord selectByPk(String schemaName, Integer objectId)
	{
		...
		try {
			String sql = StringUtil.replace(CadCilinder3dRecord.sqlSelectByPk, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			if( rs.next() )
				result = new CadCilinder3dRecord(rs);
		}
		catch(Exception e) {
			e.printStackTrace();
		}		
	...
	public ArrayList<BaseObjectRecord> selectAll(String schemaName)
	{
		...
		try {
			String sql = StringUtil.replace(CadCilinder3dRecord.sqlSelectAll, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			ResultSet rs = stmt.executeQuery();
			while( rs.next() ) {
				CadCilinder3dRecord o = new CadCilinder3dRecord(rs);
				lsResult.add(o);
			}
		}
		catch(Exception e) {
	...
	public ArrayList<BaseObjectRecord> selectByRefEntityId(String schemaName, String refEntity) 
	{
		...
		try {
			String sql = StringUtil.replace(CadCilinder3dRecord.sqlSelectByReference, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer insert(String schemaName, BaseObjectRecord o) {
		CadCilinder3dRecord oEnt = (CadCilinder3dRecord)o;		
		
		BaseDao dao = this.getDao();
		Integer result = -1;
		
		try {
			String sql = StringUtil.replace(CadCilinder3dRecord.sqlInsert, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer update(String schemaName, BaseObjectRecord o) {
		...
		try {
			String sql = StringUtil.replace(CadCilinder3dRecord.sqlUpdate, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
