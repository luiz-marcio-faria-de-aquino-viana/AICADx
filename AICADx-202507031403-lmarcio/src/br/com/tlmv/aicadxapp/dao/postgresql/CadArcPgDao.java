...
public class CadArcPgDao extends CadEntityBaseDao
{
//Public
	
	/* Constructors */
	
	public CadArcPgDao(BaseDao dao)
	{
		super(dao);
	}
	...
	public BaseObjectRecord selectByPk(String schemaName, Integer objectId)
	{
		...
		try {
			String sql = StringUtil.replace(CadArcRecord.sqlSelectByPk, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public ArrayList<BaseObjectRecord> selectAll(String schemaName)
	{
		...
		try {
			String sql = StringUtil.replace(CadArcRecord.sqlSelectAll, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			ResultSet rs = stmt.executeQuery();
			while( rs.next() ) {
				CadArcRecord o = new CadArcRecord(rs);
				lsResult.add(o);
			}
		}
		catch(Exception e) {
	...
	public ArrayList<BaseObjectRecord> selectByRefEntityId(String schemaName, String refEntity) 
	{
		...
		try {
			String sql = StringUtil.replace(CadArcRecord.sqlSelectByReference, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer insert(String schemaName, BaseObjectRecord o) {
		CadArcRecord oEnt = (CadArcRecord)o;		
		
		BaseDao dao = this.getDao();
		Integer result = -1;
		
		try {
			String sql = StringUtil.replace(CadArcRecord.sqlInsert, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			stmt.setDouble(n++, oEnt.getStartAngle());
			stmt.setDouble(n++, oEnt.getEndAngle());
	...
	public Integer update(String schemaName, BaseObjectRecord o) {
		...
		try {
			String sql = StringUtil.replace(CadArcRecord.sqlUpdate, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
