...
public class CadSphere3dPgDao extends CadEntityBaseDao
{
//Public
	
	/* Constructors */
	
	public CadSphere3dPgDao(BaseDao dao)
	{
		super(dao);
	}
	...
	public BaseObjectRecord selectByPk(String schemaName, Integer objectId)
	{
		...
		try {
			String sql = StringUtil.replace(CadSphere3dRecord.sqlSelectByPk, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			if( rs.next() )
				result = new CadSphere3dRecord(rs);
		}
		catch(Exception e) {
			e.printStackTrace();
		}		
	...
	public ArrayList<BaseObjectRecord> selectAll(String schemaName)
	{
		...
		try {
			String sql = StringUtil.replace(CadSphere3dRecord.sqlSelectAll, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
			...
			ResultSet rs = stmt.executeQuery();
			while( rs.next() ) {
				CadSphere3dRecord o = new CadSphere3dRecord(rs);
				lsResult.add(o);
			}
		}
		catch(Exception e) {
	...
	public ArrayList<BaseObjectRecord> selectByRefEntityId(String schemaName, String refEntity) 
	{
		...
		try {
			String sql = StringUtil.replace(CadSphere3dRecord.sqlSelectByReference, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer insert(String schemaName, BaseObjectRecord o) {
		CadSphere3dRecord oEnt = (CadSphere3dRecord)o;		
		
		BaseDao dao = this.getDao();
		Integer result = -1;
		
		try {
			String sql = StringUtil.replace(CadSphere3dRecord.sqlInsert, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
	...
	public Integer update(String schemaName, BaseObjectRecord o) {
		...
		try {
			String sql = StringUtil.replace(CadSphere3dRecord.sqlUpdate, AppDefs.TEMPLATEDBFILE_TAGPARM_SCHEMA_NAME, schemaName);
