...
//Private
    ...
    // DRAWFLOORS
    //
    protected void drawFloors(ICadViewBase v, GeomPoint2d ptBase2dMcs, GeomPoint2d pt2dMcs, boolean bDragMode, PrepareDrawUtil prep)
    {
    	...
    	ArrayList<CadEntity> lsEntities = oBlockDef.sortAllEntities(ptCentroid, AppDefs.OBJTYPE_BIMPISO);
        ...
        for(int i = 0; i < szLsEntities; i++) {
			...
			if(oEnt.getObjType() == AppDefs.OBJTYPE_BIMPISO) 
			{
				...
				o.redraw3d(v, 0.0, null, pt2dMcs, sclFact, false, false, prep);
				continue;
			}
        }
    }
    ...
    // DRAWWALLS
    //
    protected void drawWalls(ICadViewBase v, GeomPoint2d ptBase2dMcs, GeomPoint2d pt2dMcs, boolean bDragMode, PrepareDrawUtil prep)
    {
    	CadProjectDef projDef = this.doc.getCurrProjectDef();
    	double sclFact = projDef.getScaleFactor();
    	...
    	ArrayList<CadEntity> lsEntities = oBlockDef.sortAllEntities(ptCentroid, AppDefs.OBJTYPE_BIMPAREDE);
    	int szLsEntities = lsEntities.size();
    ...
    // DRAWENTITIES
    //
    protected void drawEntities(ICadViewBase v, GeomPoint2d ptBase2dMcs, GeomPoint2d pt2dMcs, boolean bDragMode, PrepareDrawUtil prep) 
    {
    	CadProjectDef projDef = this.doc.getCurrProjectDef();
    	double sclFact = projDef.getScaleFactor();
...
//Public
	...
	public void paint(Graphics g) 
	{
		...
		this.drawFloors(v, ptBase2dMcs, pt2dMcs, bDragMode, prep);
		this.drawWalls(v, ptBase2dMcs, pt2dMcs, bDragMode, prep);
		this.drawEntities(v, ptBase2dMcs, pt2dMcs, bDragMode, prep);
	...
	public void processMouseWheel(double rotationVal)
	{
		...
		if(diff > AppDefs.MOUSEWHEEL_TIMEOUT) {
			if(rotationVal < 0.0) {
				v.zoomMoveForwardBackwardMcs(AppDefs.ZOOM_MOVEBACKWARD);
			}
			else {
				v.zoomMoveForwardBackwardMcs(AppDefs.ZOOM_MOVEFORWARD);
			}
	...
	{
		String warnmsg = String.format("ClickedButton:%s;Modifiers:%s; ", mouseButton, modifiers);
		AppError.showCmdWarn(AppDefs.DEBUG_LEVEL07, warnmsg, this.getClass());
	...
	public void processMouseDragged(MouseEvent e)
	{
		...
		if(diff > AppDefs.MOUSEDRAGGED_TIMEOUT) {
	    	...
	    	if( e.isControlDown() ) {
	    		...
	    		if(dX < 0.0) {
	    			v.zoomTiltLeftRightMcs(AppDefs.ZOOM_DIST_TILTLEFT);		    			
	    		}
	    		else {
	    			v.zoomTiltLeftRightMcs(AppDefs.ZOOM_DIST_TILTRIGHT);		    			
	    		}
	    	}
	    		...
	    		if(dY < 0.0) {
	    			v.zoomTiltUpDownMcs(AppDefs.ZOOM_DIST_TILTDOWN);
	    		}
	    		else {
	    			v.zoomTiltUpDownMcs(AppDefs.ZOOM_DIST_TILTUP);
	    		}
	    	}
	    	else {
	    		if(this.base3DMoveModePtScr == null) return;
	    		...
	    		double dY = this.curr3DMoveModePtScr.getY() - this.base3DMoveModePtScr.getY();
	    		if(dY < 0.0) {
	    			v.zoomHeightUpDownMcs(AppDefs.ZOOM_HEIGHTDOWN);
	    		}
	    		else {
	    			v.zoomHeightUpDownMcs(AppDefs.ZOOM_HEIGHTUP);
	    		}
	    	}
