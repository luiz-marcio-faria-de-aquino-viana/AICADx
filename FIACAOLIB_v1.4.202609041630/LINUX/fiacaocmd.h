...
int  citip(char *cmd)
{
  if( !strncasecmp(cmd, ID_3W, strlen(ID_3W)) ) {
    strcpy(cmd, &cmd[2]);
    return _I3W;
  }
  else if( !strncasecmp(cmd, ID_4W, strlen(ID_4W)) ) {
    strcpy(cmd, &cmd[2]);
    return _I4W;
  }
  return _IS;
...
int ccmd(char *qdr, char *cir, char *cmd, itemElem_t *pElem)
{
  ...
  char s[31], s1[31], s_tmp[31];  // variaveis auxiliares
  int itip;
  ...
  while((*s) != '\0') {
    strhead_fia(s1, s, ',');
	  if(__DEGUG_LEVEL__ != 0) {
		  printf("ListaDeComandos: %s; ComandoAtual: %s\n", s, s1);
    }
    itip = citip(s1);
    ...
    int sz = strtail_fia(s_tmp, s, ',');
    
    s[0] = '\0';
    if(sz >= 1)
      strcpy_fia(s, & s_tmp[1]);
  ...
  switch(pElem->tip) {
    ...
    case T_ILUMINACAO :
      ...
      if(pElem->primCmdElem != NULL)
	    fas = pElem->fas & (__FIA_N);
      else
	    fas = pElem->fas & (__FIA_F1 + __FIA_F2 + __FIA_F3 + __FIA_N);
      proc_fios(qdr, cir, cirOrCmd, fas);
      if((fas = pElem->fas & __FIA_TR) != 0)
    ...
    case T_COMANDO:
	  ...
	  if( !strcmp(pCElem->pCmd->primElemCmd->hnd, pElem->hnd) ) {
		...
		fas = pCElem->pCmd->fas & (__FIA_F1 + __FIA_F2 + __FIA_F3);
		proc_fios(qdr, cir, cirOrCmd, fas);
	}
	pCElem = pCElem->prox;
      }
  ...
  if(pRaiz->prox == NULL) {
	...
	if(pElem->primCmdElem != NULL)
	  if(pElem->primCmdElem->pCmd == pCmd) {
		...
		cir = pCmd->cir;
		cirOrCmd = pCmd->cmd;
	    fas = (pCmd->fas & (__FIA_F1 + __FIA_F2 + __FIA_F3)) << 4;
	    proc_fios(qdr, cir, cirOrCmd, fas);
	  }
	break;
      case T_QUADRO:
	...
	cir = pCmd->cir;
    cirOrCmd = pCmd->cmd;
	if((pCmd->fas & __FIA_F1) != 0) {
	  fas = __FIA_R4 + __FIA_R5;
	  if((pCmd->fas & __FIA_F2) != 0) {
	    fas += __FIA_R6 + __FIA_R7;
	    if((pCmd->fas & __FIA_F3) != 0)
	      fas += __FIA_R8 + __FIA_R9;
	  }
	}
	proc_fios(qdr, cir, cirOrCmd, fas);
    }
