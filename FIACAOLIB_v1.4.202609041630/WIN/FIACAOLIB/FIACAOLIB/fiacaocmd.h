...
#ifndef __FIACAOCMD_H
#define __FIACAOCMD_H
...
int  lfios_procuraItem(itemFios_t **primFios, char *qdr, char *cir, char *cirOrCmd, itemFios_t **iItem);
...
int  lcmd_procuraItem(char *qdr, char *cir, char *cmd, itemCmd_t **iItem);
...
*/
void fatalerr(int lin, int err, char* s_errmsg);
...
int ccmd(char *qdr, char *cir, char *cmd, itemElem_t *pElem);  // converte a string de comandos em lista
 ...
 */
void proc_fios(char *qdr, char *cir, char *cirOrCmd, int fios);

/* declaracao da rotina responsavel por gerar o arquivo de saida em lisp
*/
void gerar_arqvsaidalisp(char *arqv);

/* declaracao da rotina responsavel por gerar o arquivo de saida em xml
*/
void gerar_arqvsaidaxml(char *arqv);
...
itemHnd_t* lhnd_insereItem(itemHnd_t item)
{
  ...
  if((ip = (itemHnd_t *) malloc(sizeof(itemHnd_t))) == NULL)
    fatalerr(161, ERR_ALLOCMEM, NULL);
  if((ip->hnd = (char *) malloc(STRSZ(item.hnd))) == NULL) {
    free(ip);
    fatalerr(429, ERR_ALLOCMEM, NULL);
  }
...
int  lfios_procuraItem(itemFios_t **primFios, char *qdr, char *cir, char *cirOrCmd, itemFios_t **iItem)
{
  (*iItem) = (*primFios);
  while((*iItem) != NULL) {
	if (!strcmp((*iItem)->qdr, qdr) && 
		!strcmp((*iItem)->cirOrCmd, cirOrCmd))
      return RTNORM;
    (*iItem) = (*iItem)->prox;
  }
  return RTERR;
...
itemQdr_t* lqdr_insereItem(itemQdr_t item)
{
  ...
  if((ip = (itemQdr_t *) malloc(sizeof(itemQdr_t))) == NULL)
    fatalerr(238, ERR_ALLOCMEM, NULL);
  if((ip->qdr = (char *) malloc(STRSZ(item.qdr))) == NULL) {
    free(ip);
    fatalerr(240, ERR_ALLOCMEM, NULL);
  }
  if((ip->org = (char *) malloc(STRSZ(item.org))) == NULL) {
    free(ip->qdr);
    free(ip);
    fatalerr(245, ERR_ALLOCMEM, NULL);
  }
  if((ip->cir = (char *) malloc(STRSZ(item.cir))) == NULL) {
    free(ip->org);
    free(ip->qdr);
    free(ip);
    fatalerr(339, ERR_ALLOCMEM, NULL);
  }
  if((ip->hnd = (char *) malloc(STRSZ(item.hnd))) == NULL) {
    free(ip->org);
    free(ip->qdr);
    free(ip->cir);
    free(ip);
    fatalerr(530, ERR_ALLOCMEM, NULL);
  }
  strcpy(ip->hnd, item.hnd);
  ip->idx = item.idx;
  strcpy(ip->qdr, item.qdr);
...
itemEletr_t* leltr_insereItem(itemEletr_t item)
{
  ...
  if((ip = (itemEletr_t *) malloc(sizeof(itemEletr_t))) == NULL)
    fatalerr(289, ERR_ALLOCMEM, NULL);
  ip->flg  = item.flg;
  if((ip->hnd = (char *) malloc(STRSZ(item.hnd))) == NULL) {
    free(ip);
    fatalerr(584, ERR_ALLOCMEM, NULL);
  }
  if((ip->hnd1 = (char *) malloc(STRSZ(item.hnd1))) == NULL) {
    free(ip->hnd);
    free(ip);
    fatalerr(589, ERR_ALLOCMEM, NULL);
  }
  if((ip->hnd2 = (char *) malloc(STRSZ(item.hnd2))) == NULL) {
    free(ip->hnd);
    free(ip->hnd1);
    free(ip);
    fatalerr(595, ERR_ALLOCMEM, NULL);
  }
  strcpy(ip->hnd, item.hnd);
...
itemElemCmd_t* lecmd_insereItem(itemElemCmd_t **primElemCmd, itemElemCmd_t item)
{
  ...
  if((ip = (itemElemCmd_t *) malloc(sizeof(itemElemCmd_t))) == NULL)
    fatalerr(508, ERR_ALLOCMEM, NULL);
  if((ip->hnd = (char *) malloc(STRSZ(item.hnd))) == NULL) {
    free(ip);
    fatalerr(654, ERR_ALLOCMEM, NULL);
  }
  strcpy(ip->hnd, item.hnd);
  ip->idx = item.idx;
  ip->itip = item.itip;
...
itemCmd_t* lcmd_insereItem(itemCmd_t item)
{
  ...
  if((ip = (itemCmd_t *) malloc(sizeof(itemCmd_t))) == NULL)
    fatalerr(324, ERR_ALLOCMEM, NULL);

  if((ip->cmd = (char *) malloc(STRSZ(item.cmd))) == NULL) {
    free(ip);
    fatalerr(327, ERR_ALLOCMEM, NULL);
  }
  ...
  if(item.cir == NULL) {
    ...
    if((ip->cir = (char *) malloc(STRSZ(item.cir))) == NULL) {
      free(ip->cmd);
      free(ip);
      fatalerr(333, ERR_ALLOCMEM, NULL);
    }
  }
...
itemCmd_t* lcmd_alteraItem(itemCmd_t item)
{
  ...
  if(lcmd_procuraItem(item.qdr, item.cir, item.cmd, &ip) == RTERR) return RTERR;
  ...
  if (item.qdr == NULL)
	  ip->qdr = NULL;
  else if ((ip->qdr = (char *)malloc(STRSZ(item.qdr))) == NULL)
	  fatalerr(333, ERR_ALLOCMEM, NULL);
  ...
  if(item.cir == NULL)
    ip->cir = NULL;
	 ...
	 fatalerr(334, ERR_ALLOCMEM, NULL);
...
itemDes_t* ldes_insereItem(itemDes_t item)
{
  ...
  if((ip = (itemDes_t*) malloc(sizeof(itemDes_t))) == NULL)
    fatalerr(746, ERR_ALLOCMEM, NULL);
  if((ip->des = (char*) malloc(STRSZ(item.des))) == NULL) {
    free(ip);
    fatalerr(749, ERR_ALLOCMEM, NULL);
  }
...
itemElemDes_t* ldese_insereItem(itemElemDes_t** primElemDes, itemElemDes_t item)
{
  ...
  if((ip = (itemElemDes_t*) malloc(sizeof(itemElemDes_t))) == NULL)
    fatalerr(788, ERR_ALLOCMEM, NULL);
  ip->flg = 0;
  ip->pElem = item.pElem;
  ip->prox = (*primElemDes);
  (*primElemDes) = ip;
...
itemElem_t* lelem_insereItem(itemElem_t item)
{
  ...
  if((ip = (itemElem_t *) malloc(sizeof(itemElem_t))) == NULL)
    fatalerr(376, ERR_ALLOCMEM, NULL);
  if((ip->org = (char *) malloc(STRSZ(item.org))) == NULL) {
    free(ip);
    fatalerr(379, ERR_ALLOCMEM, NULL);
  }
  if((ip->des = (char *) malloc(STRSZ(item.des))) == NULL) {
    free(ip->org);
    free(ip);
    fatalerr(722, ERR_ALLOCMEM, NULL);
  }
  if((ip->cir = (char *) malloc(STRSZ(item.cir))) == NULL) {
    free(ip->org);
    free(ip->des);
    free(ip);
    fatalerr(384, ERR_ALLOCMEM, NULL);
  }
  if((ip->hnd = (char *) malloc(STRSZ(item.hnd))) == NULL) {
    free(ip->org);
    free(ip->des);
    free(ip->cir);
    free(ip);
    fatalerr(927, ERR_ALLOCMEM, NULL);
  }
  strcpy(ip->hnd, item.hnd);
  ip->idx     = item.idx;
  ip->tip     = item.tip;
...
itemMalha_t* lmlh_insereItem(itemMalha_t **primMalha, itemMalha_t item)
{
  ...
  if((ip = (itemMalha_t *) malloc(sizeof(itemMalha_t))) == NULL)
    fatalerr(473, ERR_ALLOCMEM, NULL);
  ip->pElem = item.pElem;
...
itemPilha_t* pilha_insereItem(itemEletr_t *pEletr)
{
  ...
  if((ip = (itemPilha_t *) malloc(sizeof(itemPilha_t))) == NULL)
    fatalerr(527, ERR_ALLOCMEM, NULL);
  ip->pEletr = pEletr;
  ip->prox = topoPilha;
  topoPilha = ip;
  return ip;
...
int  citip(char *cmd)
{
  if( !strncasecmp_fia(cmd, (char*)ID_3W, (int)strlen(ID_3W)) ) {
    strcpy(cmd, &cmd[2]);
    return _I3W;
  }
  else if( !strncasecmp_fia(cmd, (char*)ID_4W, (int)strlen(ID_4W)) ) {
    strcpy(cmd, &cmd[2]);
    return _I4W;
  }
  return _IS;
...
int ccmd(char *qdr, char *cir, char *cmd, itemElem_t *pElem)
{
  ...
  while((*s) != '\0') {
    strhead_fia(s1, s, ',');
    itip = citip(s1);
    if(lcmd_procuraItem(qdr, cir, s1, &pCmd) == RTERR) {
      iCmd.cmd = s1;
      switch(pElem->tip) {
	case T_CARGA :
	case T_CAMPAINHA :
...
int ler_arqventr(char *arqv)
{
  ...
  if((f_in = fopen(arqv, "r")) == NULL)
    fatalerr(555, ERR_OPENFILE, arqv);

  if((f_tmp = fopen(FILE_TEMP, "wb")) == NULL)
    fatalerr(558, ERR_OPENFILE, (char*)FILE_TEMP);
  ...
  while(fgets(buff, sizeof(str_t), f_in) != NULL) {
	...
	fatalerr(579, ERR_CANTWRITE, (char*)FILE_TEMP);
    ...
    if(lhnd_procuraItem(iElem.hnd2, idx2, &pHnd) == RTERR) {
      ...
      if(fwrite(&R_tmp, sizeof(tempReg_t), 1, f_tmp) == 0)
	fatalerr(604, ERR_CANTWRITE, (char*)FILE_TEMP);
...
int ler_arqvtemp(itemQdr_t *pQdr)
{
  ...
  if((f_tmp = fopen(FILE_TEMP, "rb")) == NULL)
    fatalerr(637, ERR_OPENFILE, (char*)FILE_TEMP);
  ...
  while(fread(&R_tmp, sizeof(tempReg_t), 1, f_tmp) != 0) {
    if( !strcmp(R_tmp.org, pQdr->qdr) ) {
      ...
      iElem.primCmdElem = NULL;
      iElem.fas     = R_tmp.fas;
      ccmd(R_tmp.org, R_tmp.cir, R_tmp.cmd, &iElem);
      lelem_insereItem(iElem);
    }
  }
  fclose(f_tmp);
  ...
  while(pCmd != NULL) {
    if(pCmd->cir == NULL) {
      ...
      if((iCmd.cir = (char *) malloc(STRSZ(pCmd->cmd) + 1)) == NULL)
     	fatalerr(1117, ERR_ALLOCMEM, NULL);
      strcat(strcpy(iCmd.cir, "?"), pCmd->cmd);
      if((iCmd.qdr = (char *) malloc(STRSZ(pCmd->qdr) + 1)) == NULL)
		fatalerr(1118, ERR_ALLOCMEM, NULL);
	  strcpy(iCmd.qdr, pCmd->qdr);
	  iCmd.fas = pCmd->fas;
      iCmd.primElemCmd = pCmd->primElemCmd;
      lcmd_alteraItem(iCmd);
    }
...
void cons_malha(char *hnd, itemMalha_t **pItem)
{
  if((*pItem) == NULL) {
    if(((*pItem) = (itemMalha_t *) malloc(sizeof(itemMalha_t))) == NULL)
      fatalerr(744, ERR_ALLOCMEM, NULL);
    (*pItem)->pElem     = NULL;
    (*pItem)->pEletr    = NULL;
...
void proc_itemDoQuadro(itemQdr_t *pRaiz, itemElem_t *pElem)
{
  ...
  switch(pElem->tip) {
    ...
    case T_ILUMINACAO :
      ...
      proc_fios(qdr, cir, cirOrCmd, fas);
      if((fas = pElem->fas & __TR) != 0)
	proc_fios(qdr, (char*)"?", (char*)"", fas);
      break;
    case T_QUADRO:
	  qdr = pElem->org;
	  cir = pElem->cir;
...
void proc_fios(char *qdr, char *cir, char *cirOrCmd, int fios)
{
  ...
  while(pPilha != NULL) {
    if(pPilha->pEletr != NULL)
      {
      if(lfios_procuraItem(& (pPilha->pEletr)->primFios, qdr, cir, cirOrCmd, & pFios) == RTNORM) {
...
void gerar_arqvsaidalisp(char *arqv)
{
  ...
  if((f_out = fopen(arqv, "w")) == NULL)
    fatalerr(1008, ERR_OPENFILE, arqv);
  pEletr = primEletr;
  while(pEletr != NULL) {
    fprintf(f_out, "(\"%s\" (", pEletr->hnd);
    pFios = pEletr->primFios;
    while(pFios != NULL) {
      fprintf(f_out, "(\"%s\" \"%s\" \"%s\" %d)", pFios->qdr, pFios->cir, pFios->cirOrCmd, pFios->fios);
      pFios = pFios->prox;
    }
    fprintf(f_out, "))\n");
    pEletr = pEletr->prox;
...
void gerar_arqvsaidaxml(char *arqv)
{
	...
	if ((f_out = fopen(arqv, "w")) == NULL)
		fatalerr(1008, ERR_OPENFILE, arqv);
	pEletr = primEletr;
	fprintf(f_out, "<ResultadoImportaFiacao>");
	while (pEletr != NULL) {
		fprintf(f_out, "<ImportaFiacao>");
		fprintf(f_out, "<Hnd>%s</Hnd>", pEletr->hnd);
		pFios = pEletr->primFios;
		while (pFios != NULL) {
			fprintf(f_out, "<ImportaFiacaoEletroduto>");
			fprintf(f_out, "<Qdr>%s</Qdr><Cir>%s</Cir><Lbl>%s</Lbl><Fia>%d</Fia>", pFios->qdr, pFios->cir, pFios->cirOrCmd, pFios->fios);
			fprintf(f_out, "</ImportaFiacaoEletroduto>");
			pFios = pFios->prox;
		}
		fprintf(f_out, "</ImportaFiacao>");
...
void debug_comandos(void)
{
  ...
  pCmd = primCmd;
  while(pCmd != NULL) {
    writeLog("\ncmd = %s, qdr = %s, cir = %s, fas = %d", pCmd->cmd, pCmd->qdr, pCmd->cir, pCmd->fas);
...
void debug_malha(itemMalha_t *primMalha)
{
  ...
  if(primMalha->pElem == NULL)
    writeLog("nil");
	 ...
	 writeLog("%s(%d),", primMalha->pElem->hnd, primMalha->pElem->idx);
  if(primMalha->pEletr == NULL)
    writeLog("nil");
  else if(primMalha->pEletr->hnd[0] == '\0')
	 writeLog("nul");
       else
	 writeLog("%s", primMalha->pEletr->hnd);
...
void debug_result(void)
{
  ...
  while(pEletr != NULL) {
    ...
    pFios = pEletr->primFios;
    while(pFios != NULL) {
      writeLog("%s, %s, %s, %d / ", pFios->qdr, pFios->cir, pFios->cirOrCmd, pFios->fios);
      pFios = pFios->prox;
    }
    pEletr = pEletr->prox;
  }
