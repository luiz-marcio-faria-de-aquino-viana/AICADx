...
void str2fileReg(char *s, fileReg_t *reg) {
  ...
  strpiece_fia(s1, s, 1, '@');   // handle do eletroduto
  strncpy(reg->hnd, s1, 16);
  reg->hnd[16] = '\0';

  /* caracteristicas do primeiro ponto conectado ao eletroduto */
  strpiece_fia(s1, s, 1, '@');   // handle
  strncpy(reg->hnd1, s1, 16);
  reg->hnd1[16] = '\0';
  strpiece_fia(s1, s, 1, '@');   // indice
  strncpy(reg->idx1, s1, 1);
  reg->idx1[1] = '\0';
  strpiece_fia(s1, s, 1, '@');   // tipo de elemento
  strncpy(reg->tip1, s1, 11);
  reg->tip1[11] = '\0';
  strpiece_fia(s1, s, 1, '@');   // nome do quadro (se tipo = EQUADRO)
  strncpy(reg->qdr1, s1, 20);
  reg->qdr1[20] = '\0';
  strpiece_fia(s1, s, 1, '@');   // quadro de origem
  strncpy(reg->org1, s1, 20);
  reg->org1[20] = '\0';
  strpiece_fia(s1, s, 1, '@');   // identificacao do desvio (se tipo = EDESVIO)
  strncpy(reg->des1, s1, 20);
  reg->des1[20] = '\0';
  strpiece_fia(s1, s, 1, '@');   // identificacao do circuito (se tipo = ECARGA)
  strncpy(reg->cir1, s1, 3);
  reg->cir1[3] = '\0';
  strpiece_fia(s1, s, 1, '@');   // identificacao dos comandos (se tipo != EQUADRO)
  strncpy(reg->cmd1, s1, 30);
  reg->cmd1[30] = '\0';
  strpiece_fia(s1, s, 1, '@');   // sistema de fases
  ...
  reg->org2[20] = '\0';
  strpiece_fia(s1, s, 1, '@');   // identificacao do circuito (se tipo = EDESVIO)
